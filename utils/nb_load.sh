...
usage() {
  echo "Usage: $0 --submissions <submissions_dir> --assignment <assignment_name> --idlist <idlist_file>"
  exit 1
}
...
NBGRADER_ASSIGNMENTS_DIR="$GITPOD_REPO_ROOT/nbgrader/Assignments/submitted"
...
for file in "$SUBMISSIONS_DIR"/*.ipynb; do
  if [[ -f "$file" ]]; then
    # Extract the first instance of _[0-9]*_ from the filename
    ...
    if [[ -n "$SID" ]]; then
      ...
      if [[ -d "$STUDENT_DIR" ]]; then
        ...
        if [[ -f "$TARGET_FILE" ]]; then
          echo "File $TARGET_FILE already exists, deleting file from submissions directory."
          rm "$file"
        else
